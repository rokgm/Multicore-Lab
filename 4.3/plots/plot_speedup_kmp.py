...
n_threads = np.array([1, 2, 4, 8, 16, 24, 32, 48])
...
# reading files
text_file = open("4.3/measurements/2000_compact.txt", "r")
lines = text_file.read().split('\n')
lines = [float(i) for i in lines[:-1]]
groups = [lines[x:x+5] for x in range(0, len(lines), 5)]
runtime_par2000_compact = np.array([sum(group)/len(group) for group in groups])
speedups2000_compact = runtime_seq2000 / runtime_par2000_compact

text_file = open("4.3/measurements/2000_scatter.txt", "r")
lines = text_file.read().split('\n')
lines = [float(i) for i in lines[:-1]]
groups = [lines[x:x+5] for x in range(0, len(lines), 5)]
runtime_par2000_scatter = np.array([sum(group)/len(group) for group in groups])
speedups2000_scatter = runtime_seq2000 / runtime_par2000_scatter

text_file = open("4.3/measurements/6000_compact.txt", "r")
lines = text_file.read().split('\n')
lines = [float(i) for i in lines[:-1]]
groups = [lines[x:x+5] for x in range(0, len(lines), 5)]
runtime_par6000_compact = np.array([sum(group)/len(group) for group in groups])
speedups6000_compact = runtime_seq6000 / runtime_par6000_compact

text_file = open("4.3/measurements/6000_scatter.txt", "r")
lines = text_file.read().split('\n')
lines = [float(i) for i in lines[:-1]]
groups = [lines[x:x+5] for x in range(0, len(lines), 5)]
runtime_par6000_scatter = np.array([sum(group)/len(group) for group in groups])
