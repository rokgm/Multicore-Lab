...
void MinimaxStrategy::searchBestMove()
{
    ...
    currentValue = minimax(&_currentBestMove, 0, maximizingPlayer);
...
int MinimaxStrategy::minimax(Move &m, int depth, bool maximizingPlayer)
{
    ...
    if (depth == _maxDepth) // TODO fix condition
    {
        playMove(m);
        value = evaluate();
    ...
    if (maximizingPlayer)
    {
        ...
        {
            value = std::max(currentValue, minimax(m, depth + 1, false));
            return value;
        }
    }
    else
    {
        ...
        while (list.getNext(m))
        {
            value = std::min(currentValue, minimax(m, depth + 1, true, maxDepth));
            return value;
        }
    }
