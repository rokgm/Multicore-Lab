/**
 * Negamax with alpha-beta pruning search strategy.
 */
...
public:
    ...
    SearchStrategy *clone() override { return new ABStrategy(); }
...
void ABStrategy::searchBestMove()
{
    ...
    for (int depth = 1; depth <= _maxDepth; depth++) {
		...
		alphabeta(0, minEvaluation(), maxEvaluation());
