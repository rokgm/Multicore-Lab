...
void exchange_boundaries(double *u, algoparam_t* param, local_process_info* local_process_info) {
    ...
    MPI_Sendrecv(param->send_buff_y_right, local_size_y, MPI_DOUBLE, local_process_info->rank_right, 0, 
                param->recv_buff_y_left, local_size_y, MPI_DOUBLE, local_process_info->rank_left, 0, 
    ...
    MPI_Sendrecv(param->send_buff_y_left, local_size_y, MPI_DOUBLE, local_process_info->rank_left, 1, 
                param->recv_buff_y_right, local_size_y, MPI_DOUBLE, local_process_info->rank_right, 1, 
                local_process_info->comm_cart, MPI_STATUS_IGNORE);
    // Don't copy from an empty buffer.
    if (local_process_info->rank_right != MPI_PROC_NULL) {
    ...
    // Send down to up.
    MPI_Sendrecv(&u[local_allocated_x + 1], local_size_x, MPI_DOUBLE, local_process_info->rank_up, 2, 
                 &u[local_allocated_x * (local_allocated_y - 1) + 1], local_size_x, MPI_DOUBLE, local_process_info->rank_down, 2, 
                 local_process_info->comm_cart, MPI_STATUS_IGNORE);

    // Send up to down.
    MPI_Sendrecv(&u[local_allocated_x * (local_allocated_y - 2) + 1], local_size_x, MPI_DOUBLE, local_process_info->rank_down, 3, 
                 &u[1], local_size_x, MPI_DOUBLE, local_process_info->rank_up, 3, 
                 local_process_info->comm_cart, MPI_STATUS_IGNORE);
}
